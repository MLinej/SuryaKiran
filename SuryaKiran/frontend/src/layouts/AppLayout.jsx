...
import { Copy, Activity, AlertTriangle, MessageSquare, LogOut, Wrench, Zap, FileText } from 'lucide-react';
...
function initGoogleTranslate() {
    ...
    new window.google.translate.TranslateElement(
        {
            pageLanguage: 'en',
            includedLanguages: 'en,hi,gu,es,fr,de,zh-CN,ja,ar,pt,ta,te,kn,mr',
...
export default function AppLayout() {
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f7fa', fontFamily: "'DM Sans', sans-serif" }}>
            <aside style={{ width: 260, background: '#0f172a', color: 'white', padding: '24px 0', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
                <div style={{ padding: '0 24px', marginBottom: 40, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, background: 'linear-gradient(135deg,#f59e0b,#d97706)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>SK</div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.path} to={item.path} style={{
                                ...
                                textDecoration: 'none', background: isActive ? 'rgba(245,158,11,0.15)' : 'transparent',
                                color: isActive ? '#fcd34d' : '#94a3b8', fontWeight: isActive ? 600 : 500, transition: 'all 0.2s'
                            }}>
                ...
                <div style={{ marginTop: 'auto', padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <Link to='/' style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, textDecoration: 'none', color: '#94a3b8', fontWeight: 500 }}>
                        <Copy className='w-5 h-5' style={{ color: '#64748b' }} />
                        Back to Site
                    </Link>
                    <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, border: 'none', background: 'transparent', cursor: 'pointer', color: '#94a3b8', fontWeight: 500, textAlign: 'left' }}>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <header style={{ height: 66, background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 32px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        <div id='google_translate_element' style={{ marginRight: 8, minWidth: 160 }} />
