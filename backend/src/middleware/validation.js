...
const predictionSchema = z.object({
    ...
    inverter_temp_c: z.number().min(-20).max(200),
    ambient_temp_c: z.number().min(-20).max(80),
    ac_voltage_v: z.number().min(0).max(5000),
    dc_voltage_v: z.number().min(0).max(5000),
    ac_current_a: z.number().min(0).max(10000),
    dc_current_a: z.number().min(0).max(10000),
